...
(async()=>{const browser=await chromium.launch({headless:true,channel:process.env.BROWSER_CHANNEL||'msedge'});try{
 ...
 await page.click('#bookMediaTab');assert.equal(await page.locator('.bookVideos iframe').count(),1);assert.match(await page.locator('.bookVideos').innerText(),/主題曲及MV/);await page.click('#closeModal');assert.equal(await page.locator('#bookshopHub:visible').count(),1);
