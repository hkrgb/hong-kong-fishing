...
function showTackle(){
 const w=wallet();modal('海港釣具店','<div class="tackleShop" style="background-image:linear-gradient(#071d2966,#071d29aa),url('+shopAsset('bait-kit')+')"><div class="shopOffers"></div><p class="baitNote">每次拋竿用 1 份魚餌，空手或斷線亦會消耗。</p></div>');
...
function confirmSellAll(){
 const {sold,released}=wallet(),seen=new Set(),items=save.bag.filter(f=>{const k=CoastEconomy.catchKey(f);if(sold.has(k)||released.has(k)||seen.has(k))return false;seen.add(k);return true;}).map(f=>({key:CoastEconomy.catchKey(f),amount:salePrice(f)}));
